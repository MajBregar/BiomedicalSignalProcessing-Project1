...
function [idx] = Backup(file, sampling_rate, plots_enabled)
    S = load(file);
...
function QRS_locs = peak_detection(filt, mwa, fs, RP_ms, Tw_ms)
    ...
    for n = 3:length(mwa)
        ...
        is_QRS_candidate = peakF > THF1 && peakI > THI1 && (peak_index - last_QRS) > refractory_samples;
        ...
        if is_QRS_candidate
            ...
            if dt < T_wave_period_samples && last_QRS > 10
                % slope of last QRS
        ...
        if (n - last_QRS) > RR_missed
            ...
            window_start = last_QRS + refractory_samples;
            window_end   = n - 1;
